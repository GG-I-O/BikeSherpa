...
public record GetAllMyDeliveriesQuery(
     string UserEmail,
     DateTimeOffset Date
) : IQuery<GetAllDailyDeliveriesResult>;

public class GetAllMyDeliveriesHandler(
     IReadRepository<Courier> courierRepository,
     IReadRepository<Delivery> deliveryRepository
     ): IQueryHandler<GetAllMyDeliveriesQuery, GetAllDailyDeliveriesResult>
{
     
     public async ValueTask<GetAllDailyDeliveriesResult> Handle(GetAllMyDeliveriesQuery request, CancellationToken cancellationToken)
     {
