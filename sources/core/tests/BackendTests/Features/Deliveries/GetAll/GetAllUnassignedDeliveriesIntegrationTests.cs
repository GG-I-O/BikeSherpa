...
[TestSubject(typeof(GetAllUnassignedDeliveriesEndpoint))]
[Trait("Category", "Integration")]
public class GetAllUnassignedDeliveriesIntegrationTests : IClassFixture<IntegrationTestWebApplicationFactory>
{
     ...
     public GetAllUnassignedDeliveriesIntegrationTests(IntegrationTestWebApplicationFactory factory)
     {
     ...
     [Fact]
     public async Task ShouldReturnDailySteps_ForUnassignedDeliveriesOnDate()
     {
          // Arrange
          ...
          {
               // Act
               var response = await client.GetAsync($"/api/deliveries/unassignedDeliveries/{date:O}", CancellationToken.None);
               ...
               stepObject.GetProperty("courierId").ValueKind.Should().Be(JsonValueKind.Null);
          }
          finally
          {
               // Clean
