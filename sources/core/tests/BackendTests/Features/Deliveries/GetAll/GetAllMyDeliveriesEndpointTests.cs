...
[UsedImplicitly]
public class GetAllMyStepsWebApplicationFactory() : TestWebApplicationFactory("read:myDeliveries", "read:myDeliveries", "mockEmail@mail.com");

public class GetAllMyDeliveriesEndpointTests(
     GetAllMyStepsWebApplicationFactory factory,
     ITestContextAccessor testContextAccessor) : IClassFixture<GetAllMyStepsWebApplicationFactory>
{
     ...
     public async Task HandleAsync_ShouldCallMediatorAndSendResponse_WhenDailyStepsExist()
     {
          ...
          _mockMediator
               .Setup(m => m.Send(
                    It.IsAny<GetAllMyDeliveriesQuery>(),
                    It.IsAny<CancellationToken>()))
               .ReturnsAsync(new GetAllDailyDeliveriesResult.Success(expectedDeliveries));

          // Act
          var response = await _client.GetAsync($"/api/deliveries/myDeliveries/{date:O}", _cancellationToken);
          ...
          _mockMediator.Verify(
               m => m.Send(
                    It.Is<GetAllMyDeliveriesQuery>(q => q.Date == date),
                    It.IsAny<CancellationToken>()),
               Times.Once);
     ...
     public async Task HandleAsync_ShouldThrowUnauthorizedAccessException_WhenMediatorReturnsUnauthorized()
     {
          ...
          // Act
          var act = async () => await _client.GetAsync($"/api/deliveries/myDeliveries/{date:O}", _cancellationToken);
