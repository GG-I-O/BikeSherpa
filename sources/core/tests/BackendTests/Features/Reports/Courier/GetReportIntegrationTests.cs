...
public class GetReportIntegrationTests : IClassFixture<IntegrationTestWebApplicationFactory>
{
     ...
     public async Task ShouldReturnReport_ForCustomerAndDateRange()
     {
          ...
          try
          {
               ...
               report.Deliveries[0].DeliveryLabel.Should()
                    .Be(
                         $"Livraison n° {delivery.Code} le {delivery.StartDate.Day:D2}/{delivery.StartDate.Month:D2}{(!string.IsNullOrEmpty(delivery.CustomerReference) ? $" - Ref client : {delivery.CustomerReference}" : "")}");
